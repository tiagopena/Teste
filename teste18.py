...
for item01 in topologia_final_smartlog['localidade']:
    for item02 in item01['equipamento']:
        for item03 in item02['par']:
            for item04 in topologia_original_smart:
                for item05 in item04['testes']:
                    if item05['tipo'] == 'bgp':
                        for item06 in item05['ip_vizinhos']:
                            if (item04['grupo'] == item01['cidade']) and (item02['host'] == item04['nome']) and (item03['membro'] == item06.split('#')[1].split(' - ')[0]):
                                #print(item06.split('#')[0])
                                item03['ip_vizinho'] = item06.split('#')[0]
